document.addEventListener("DOMContentLoaded", () => {
    ...
    if (loader) {
        ...
        const enterSite = () => {
            if (closed) return;
            closed = true;
            loader.classList.add("swipe-away");
            document.body.classList.remove("intro-active");
            setTimeout(() => loader.remove(), 950);
        };

        loader.addEventListener("touchstart", (event) => {
            touchStartY = event.touches[0]?.clientY ?? null;
        }, { passive: true });

        loader.addEventListener("touchend", (event) => {
            if (touchStartY === null) return;
            const endY = event.changedTouches[0]?.clientY ?? touchStartY;
            if (touchStartY - endY > 55) enterSite();
            touchStartY = null;
        }, { passive: true });

        loader.addEventListener("wheel", (event) => {
            if (event.deltaY > 25) enterSite();
        }, { passive: true });

        loader.addEventListener("click", (event) => {
            if (event.target.closest("#intro-enter-cue")) enterSite();
        });

        document.addEventListener("keydown", (event) => {
            if (["ArrowUp", "Enter", " "].includes(event.key)) enterSite();
        });
    }
